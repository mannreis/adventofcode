    ...
    a.iter().enumerate().for_each(|(i,f)|{
        ...
        while let Some(s) = _n.next() {
            ...
            if f.0 == s.0 {
                ...
                //println!("{:?} + {:?} = {:?}",f,s,d);
                if contains(&m, f.1 as isize + d.0 as isize, f.2 as isize + d.1 as isize) {
                    //println!("Counting {:?}",(f.1 as isize + d.0, f.2+d.1 as isize));
                    r.insert((f.1 as isize + d.0, f.2+d.1 as isize));
                }
                if contains(&m, s.1 as isize - d.0 as isize, s.2 as isize - d.1 as isize) {
                    //println!("Counting {:?}",(s.1 as isize - d.0 as isize, s.2 as isize - d.1 as isize));
                    r.insert((s.1 as isize - d.0 as isize, s.2 as isize - d.1 as isize));
                }
            }
        }
    });
